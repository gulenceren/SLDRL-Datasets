...
b = load("../DataSets/8PathsAgent1.txt"); % Observing pre-defined trajectories
a = a(:,1:showSize);
b = b(:,1:showSize);
figure
hold on;
...
legend('Pure DRL Agent','SLDRL Agent - Predefined Trajectories');
xlabel('Time');
ylabel('Best Path Length');
ylim([8, max([mean(a, 1), mean(b, 1)]) + 1]); % Y eksenini 0'dan başlat
%xlim([0 80]);
title('Best Path Length - Predefined Trajectories');
