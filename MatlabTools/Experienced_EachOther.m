...
b = load("../DataSets/ExperiencedAgent4.txt"); % Observing an experienced agent
...
plot(mean(c,1),'LineWidth', lineSize, 'Color', [0 0 0], 'Linestyle', '-.');
...
legend('Pure DRL Agent','SLDRL Agent - Observing Experienced Agent', 'SLDRL Agent - Observing Inexperienced Agent');
xlabel('Time');
ylabel('Best Path Length');
ylim([8, max([mean(a, 1), mean(b, 1)]) + 1]); % Y eksenini 0'dan başlat
title('Performance of SLDRL Agent in Different Experiement Setups');
